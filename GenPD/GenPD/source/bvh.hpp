...
#define MIN(a,b) ((a) < (b) ? (a) : (b))
	...
	{
	public:
		FORCEINLINE static void getDistances(const vec3f &p, float &d3, float &d4, float &d5, float &d6, float &d7,
			float &d8) {
			d3 = p[0] + p[1];
			d4 = p[0] + p[2];
			d5 = p[1] + p[2];
	...
	public:
		...
		FORCEINLINE bool inside(const vec3f &p) const {
			for (int i = 0; i < 3; i++) {
				if (p[i] < _dist[i] || p[i] > _dist[i + 9]) return false;
			}

			float d[6];
			getDistances(p, d);
			for (int i = 3; i < 9; i++) {
				if (d[i - 3] < _dist[i] || d[i - 3] > _dist[i + 9]) return false;
			}
		...
		FORCEINLINE float center(int i) const { return (_dist[i + 9] + _dist[i]) * 0.5f; }
...
#define BOX kDOP18
	...
	public:
		...
		FORCEINLINE bool isRoot() { return _parent == NULL; }//1Ç≈ÇÈÅ[Ç∆
