...
std::pair<Vec3, double> Quaternion::to_axisangle() const {
	...
	return std::pair<Vec3, double>(axis, angle);
}
...
Transformation Transformation::operator/(double s) const { return (*this) * (1. / s); }
...
Quaternion Quaternion::operator/(double s) const { return (*this) * (1. / s); }

Vec3 Quaternion::rotate(const Vec3 &x) const {
	return x * (sq(s) - dot(v, v)) + 2. * v * dot(v, x) + 2. * cross(v, x) * s;
}

Vec3 Transformation::apply(const Vec3 &x) const { return translation + scale * rotation.rotate(x); }

Vec3 Transformation::apply_vec(const Vec3 &v) const { return rotation.rotate(v); }

double norm2(const Quaternion &q) { return sq(q.s) + norm2(q.v); }
...
DTransformation get_dtrans(const Motion &motion, double t) {
	...
	double qq = sq(q.s) + norm2(q.v), qdq = q.s * dq.s + dot(q.v, dq.v);
	double normq = sqrt(qq);
	T.rotation = q / normq;
	dT.rotation = dq / normq - q / normq * qdq / qq;
	return std::make_pair(T, dT);
}
...
Vec3 apply_dtrans(const DTransformation &dtrans, const Vec3 &x0, Vec3 *vel) {
	...
	if (vel) {
		...
		*vel = dT.translation + dT.scale * T.rotation.rotate(x0) + T.scale * cross(w, T.rotation.rotate(x0));
	}
	return x;
}

Vec3 apply_dtrans_vec(const DTransformation &dtrans, const Vec3 &v0) { return dtrans.first.apply_vec(v0); }
}